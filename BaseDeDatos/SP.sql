...
BEGIN
    BEGIN TRY
        ...
        WHERE CP.ID_USUARIO_CUIDADOR = @ID_CUIDADOR AND FEC_FIN IS NULL;
...
BEGIN
    BEGIN TRY
        ...
        BEGIN
            ...
            RETURN;
        END
		IF NOT EXISTS (SELECT 1 FROM CUIDADOR_PACIENTE WHERE [ID_USUARIO_CUIDADOR] = @ID_CUIDADOR AND [ID_USUARIO_PACIENTE] = @ID_PACIENTE AND FEC_FIN IS NULL)
		BEGIN
            SELECT @ERROR_ID = NUM_ERROR, 
                   @ERROR_CODE = CODE_ERROR, 
                   @ERROR_DESCRIPTION = DESCRIPTION_ERROR
        ...
        WHERE P.ID_USUARIO = @ID_PACIENTE AND J.ID_JUEGO = @ID_JUEGO
...
-- SP031: INSERTAR PUNTAJE
CREATE OR ALTER PROCEDURE SP_INSERTAR_PUNTAJE
	@ID_JUEGO INT,
	@ID_PACIENTE INT,
	@PUNTAJE INT,
	@ID_RETURN INT OUTPUT,
...
BEGIN
	BEGIN TRY
		...
		IF NOT EXISTS(SELECT 1 FROM JUEGO_PACIENTE WHERE ID_PACIENTE = @ID_PACIENTE AND @ID_JUEGO = ID_JUEGO)
		BEGIN
			SELECT @ERROR_ID = NUM_ERROR, 
                   @ERROR_CODE = CODE_ERROR, 
                   @ERROR_DESCRIPTION = DESCRIPTION_ERROR
		...
		INSERT INTO PUNTAJE(FECHA_HORA, PUNTAJE, ID_JUEGO, ID_USUARIO)
			VALUES (GETDATE(), @PUNTAJE, @ID_JUEGO, @ID_PACIENTE)
...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM USUARIO U
        LEFT JOIN PING P 
            ON P.ID_USUARIO = U.ID_USUARIO AND P.ESTADO = 1
...
BEGIN
    BEGIN TRY
        IF NOT EXISTS (SELECT 1 FROM SESION WHERE TOKEN = @TOKEN AND FECHA_FINAL IS NULL)
        BEGIN
            SELECT @ERROR_ID = NUM_ERROR, 
                   @ERROR_CODE = CODE_ERROR, 
                   @ERROR_DESCRIPTION = DESCRIPTION_ERROR
		...
		INNER JOIN PING P ON P.ID_USUARIO = U.ID_USUARIO AND P.ESTADO = 1
